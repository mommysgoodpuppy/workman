...
function mapExtractedResult(extracted: ExtractedResult): ForeignTypeResult {
  ...
  for (const entry of extracted.types) {
    if (entry.kind === "struct") {
      types.set(entry.name, buildStructInfo(entry));
  ...
  for (const fn of extracted.fns) {
    const params = fn.params.map(mapTypeDesc);
    const returnType = fn.return ? mapTypeDesc(fn.return) : { kind: "unit" };
    values.set(fn.name, {
      quantifiers: [],
...
function buildStructInfo(entry: ExtractedStruct): TypeInfo {
  const fields = new Map<string, Type>();
  const recordFields = new Map<string, number>();
  entry.fields.forEach((field, index) => {
    fields.set(field.name, mapTypeDesc(field.type));
...
function mapTypeDesc(desc: ExtractedTypeDesc): Type {
  switch (desc.kind) {
    case "void":
      return { kind: "unit" };
    case "bool":
      return { kind: "bool" };
    case "int":
      return mapIntType(desc);
    case "float":
      return mapFloatType(desc);
    case "pointer":
      return {
        kind: "constructor",
        name: "Ptr",
        args: [desc.child ? mapTypeDesc(desc.child) : unknownForeignType()],
      };
    case "optional":
      return {
        kind: "constructor",
    ...
    case "named":
      return mapNamedType(desc.name ?? "Unknown");
    case "array":
      return {
        kind: "constructor",
        name: "Ptr",
...
function mapIntType(desc: ExtractedTypeDesc): Type {
  if (desc.name) {
    return mapNamedType(desc.name);
  }
...
function mapFloatType(desc: ExtractedTypeDesc): Type {
  if (desc.name) {
    return mapNamedType(desc.name);
  }
  const bits = desc.bits ?? 32;
  return mapNamedType(`f${bits}`);
}

function mapNamedType(name: string): Type {
  switch (name) {
    case "bool":
      return { kind: "bool" };
    case "void":
