...
function formatTypeWithCarriersInternal(
  type: Type,
  context: PrintContext,
  prec: number,
): string {
  ...
  switch (type.kind) {
    case "var":
      return ensureName(context, type.id);
    case "func": {
      const left = formatTypeWithCarriersInternal(type.from, context, 1);
      const right = formatTypeWithCarriersInternal(type.to, context, 0);
      const result = `${left} -> ${right}`;
      return prec > 0 ? `(${result})` : result;
    }
    case "constructor": {
      ...
      if (
        ...
        const firstArg = formatTypeWithCarriersInternal(
          type.args[0],
          context,
          2,
        );
      ...
      const args = type.args.map((arg) =>
        formatTypeWithCarriersInternal(arg, context, 2)
      ).join(
        ", ",
      );
      return `${type.name}<${args}>`;
    ...
    case "tuple": {
      const elements = type.elements.map((el) =>
        formatTypeWithCarriersInternal(el, context, 0)
      )
        .join(", ");
      return `(${elements})`;
    }
    case "record": {
      ...
      const fields = entries
        .map(([name, fieldType]) =>
          `${name}: ${formatTypeWithCarriersInternal(fieldType, context, 0)}`
        )
        .join(", ");
      return `{ ${fields} }`;
    }
    case "effect_row": {
      ...
      const parts = entries.map(([label, payload]) =>
        payload
          ? `${label}(${formatTypeWithCarriersInternal(payload, context, 0)})`
          : label
      );

      if (flattenedType.tail) {
        const tailStr = formatTypeWithCarriersInternal(
          flattenedType.tail,
          context,
          0,
        );
